...
const DataService = (function () {
  ...
  function resolveUrls() {
    ...
    return { salesUrl: CONFIG.ACTUAL_SALES_API, targetsUrl: CONFIG.SALES_TARGET_API, summaryUrl: CONFIG.MONTHLY_SUMMARY_API, saleSummaryUrl: CONFIG.SALE_SUMMARY_API };
  }
  ...
  async function refresh() {
    ...
    const now = new Date();
    try {
      const { salesUrl, targetsUrl, summaryUrl, saleSummaryUrl } = resolveUrls();
      const [salesRaw, targetsRaw, summaryRaw, saleSummaryRaw] = await Promise.all([
        fetchJSON(salesUrl),
      ...
      const hash = simpleHash(JSON.stringify(shipments) + '|' + JSON.stringify(targetHistory) + '|' + JSON.stringify(monthlySummary) + '|' + JSON.stringify(saleSummary));
      const changed = hash !== lastHash;
      lastHash = hash;

      current = {
        shipments, targetHistory, targetsBySale, monthlySummary, saleSummary,
