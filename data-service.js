...
const DataService = (function () {
  ...
  function normalizeShipment(raw) {
    ...
    return {
      ...
      customer:    customer || 'Không xác định',
      customerCode: safeStr(raw.CustomerCode, ''),
  ...
  function resolveUrls() {
    if (CONFIG.DEMO_MODE) {
      return { salesUrl: CONFIG.DEMO_ACTUAL_SALES_FILE, targetsUrl: CONFIG.DEMO_SALES_TARGET_FILE };
    }
    return { salesUrl: CONFIG.ACTUAL_SALES_API, targetsUrl: CONFIG.SALES_TARGET_API };
  }
  ...
  async function refresh() {
    ...
    const now = new Date();
    try {
      const { salesUrl, targetsUrl } = resolveUrls();
      const [salesRaw, targetsRaw] = await Promise.all([
        fetchJSON(salesUrl),
      ...
      const hash = simpleHash(JSON.stringify(shipments) + '|' + JSON.stringify(targetHistory));
      const changed = hash !== lastHash;
      lastHash = hash;

      current = {
        shipments, targetHistory, targetsBySale,
  ...
  function addLocalTargetRevision(rawRow){
    ...
    normalized.targetId = normalized.targetId || ('TGT-LOCAL-' + Date.now());
    normalized.versionLabel = normalized.versionLabel || ('Revision ' + normalized.version);
  ...
  return { init, subscribe, getCurrent, forceRefresh, startAutoRefresh, stopAutoRefresh, addLocalTargetRevision };
})();
