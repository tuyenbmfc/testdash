...
const DataService = (function () {
  ...
  let current = {
    ...
    noidiaTarget: {},     // { [year]: { target, original, effectiveDate, reason, updatedBy } } — chỉ tiêu thị trường Nội địa (USD)
  ...
  function normalizeShipment(raw) {
    ...
    return {
      ...
      sale:        canonicalSaleName(raw.Sale) || 'Chưa gán',
  ...
  function normalizeTargetRow(raw) {
    ...
    let targetType = (ttRaw.indexOf('ROPHI') >= 0 || ttRaw.indexOf('TILAPIA') >= 0 || ttRaw === 'ROPI') ? 'ropi' : 'sale';
    const sale = canonicalSaleName(raw.Sale);
    // Chỉ tiêu thị trường Nội địa là TỔNG CHUNG theo thị trường, KHÔNG phải chỉ tiêu của 1 Sale —
    // 'Nội địa' từng bị coi là tên 1 Sale (do cột Sale trong TargetPlan ghi 'Nội địa' cho dòng chỉ
    // tiêu này), khiến trang chính đếm nhầm thành "Sale thứ 7" không tồn tại. Xử lý giống rô phi:
    // tách riêng loại 'noidia', không gộp vào chỉ tiêu theo Sale.
    if (targetType === 'sale' && sale === 'Nội địa') targetType = 'noidia';
    // Chỉ tiêu rô phi / nội địa là TỔNG CHUNG, không gắn Sale — cho phép dòng không có Sale.
    if (!sale && targetType !== 'ropi' && targetType !== 'noidia') return null;
    return {
      targetId:    safeStr(raw.TargetId, ''),
      year:        safeInt(raw.Year, 2026),
      sale:        sale || (targetType === 'ropi' ? '(Rô Phi)' : (targetType === 'noidia' ? 'Nội địa' : '')),
      targetType,
  ...
  /* Chỉ tiêu thị trường Nội địa (tổng chung/năm, doanh thu USD) — lấy phiên bản mới nhất theo
     năm. Cùng khuôn mẫu với deriveTilapiaTarget(). Trả về { [year]: { target, effectiveDate,
     reason, updatedBy } }. */
  function deriveNoiDiaTarget(history) {
    const byYear = {};
    history.filter(r => r.targetType === 'noidia').forEach(r => {
      if (!byYear[r.year]) byYear[r.year] = [];
      byYear[r.year].push(r);
    });
    const out = {};
  ...
  function deriveTargetsBySale(history) {
    ...
    history.forEach(row => {
      if (row.targetType === 'ropi' || row.targetType === 'noidia') return; // chỉ tiêu tổng chung, không phải của 1 Sale
  ...
  async function refresh() {
    ...
    try {
      ...
      const noidiaTarget = deriveNoiDiaTarget(targetHistory);
      ...
      current = {
        shipments, targetHistory, targetsBySale, tilapiaTarget, noidiaTarget, monthlySummary, saleSummary, weeklyNewContracts, productAliasRules, chemicalThresholdRules, industryNews, contractProgress,
  ...
  function applyRevisionLocally(normalized){
    ...
    const tilapiaTarget = deriveTilapiaTarget(targetHistory);
    const noidiaTarget = deriveNoiDiaTarget(targetHistory);
    current = { ...current, targetHistory, targetsBySale, tilapiaTarget, noidiaTarget, changed: true, lastRefresh: new Date() };
    subscribers.forEach(cb => { try { cb(current); } catch (e) { console.error('subscriber error', e); } });
    return normalized;
  }
