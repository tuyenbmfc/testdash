...
const DataService = (function () {
  ...
  function normalizeTargetRow(raw) {
    ...
    const ttRaw = safeStr(raw.TargetType, '').toUpperCase().replace(/\s+/g, '');
    const targetType = (ttRaw.indexOf('RÔPHI') >= 0 || ttRaw.indexOf('ROPHI') >= 0
      || ttRaw.indexOf('TILAPIA') >= 0 || ttRaw === 'ROPI') ? 'ropi' : 'sale';
