...
const DataService = (function () {
  ...
  function resolveUrls() {
    if (CONFIG.DEMO_MODE) {
      return { salesUrl: CONFIG.DEMO_ACTUAL_SALES_FILE, targetsUrl: CONFIG.DEMO_SALES_TARGET_FILE, summaryUrl: CONFIG.DEMO_MONTHLY_SUMMARY_FILE, saleSummaryUrl: CONFIG.DEMO_SALE_SUMMARY_FILE, newContractsUrl: CONFIG.DEMO_NEW_CONTRACTS_FILE, productAliasUrl: CONFIG.DEMO_PRODUCT_ALIAS_FILE, newsUrl: CONFIG.DEMO_INDUSTRY_NEWS_FILE };
    }
    return { salesUrl: CONFIG.ACTUAL_SALES_API, targetsUrl: CONFIG.SALES_TARGET_API, summaryUrl: CONFIG.MONTHLY_SUMMARY_API, saleSummaryUrl: CONFIG.SALE_SUMMARY_API, newContractsUrl: CONFIG.NEW_CONTRACTS_API, productAliasUrl: CONFIG.PRODUCT_ALIAS_API, newsUrl: CONFIG.INDUSTRY_NEWS_API };
  }
  ...
  // Bảng "ProductAliasRules": mỗi dòng là 1 quy tắc gộp tên sản phẩm — Keywords là danh sách từ
  ...
  async function refresh() {
    ...
    const now = new Date();
    try {
      const { salesUrl, targetsUrl, summaryUrl, saleSummaryUrl, newContractsUrl, productAliasUrl, newsUrl } = resolveUrls();
      const [salesRaw, targetsRaw, summaryRaw, saleSummaryRaw, newContractsRaw, productAliasRaw, newsRaw] = await Promise.all([
        fetchJSON(salesUrl),
      ...
      const hash = simpleHash(JSON.stringify(shipments) + '|' + JSON.stringify(targetHistory) + '|' + JSON.stringify(monthlySummary) + '|' + JSON.stringify(saleSummary) + '|' + JSON.stringify(weeklyNewContracts) + '|' + JSON.stringify(productAliasRules) + '|' + JSON.stringify(industryNews));
      const changed = hash !== lastHash;
      lastHash = hash;

      current = {
        shipments, targetHistory, targetsBySale, monthlySummary, saleSummary, weeklyNewContracts, productAliasRules, industryNews,
