...
const DataService = (function () {
  ...
  function normalizeTargetRow(raw) {
    ...
    // BỎ DẤU tiếng Việt trước khi so — tránh lỗi Unicode tổ hợp (NFD) khi Excel ghi "Rô Phi"
    // ở dạng chữ 'o' + dấu mũ rời (không khớp chuỗi dựng sẵn).
    const ttRaw = stripAccents(safeStr(raw.TargetType, '')).toUpperCase().replace(/\s+/g, '');
