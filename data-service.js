...
const DataService = (function () {
  ...
  function resolveUrls() {
    if (CONFIG.DEMO_MODE) {
      return { salesUrl: CONFIG.DEMO_ACTUAL_SALES_FILE, targetsUrl: CONFIG.DEMO_SALES_TARGET_FILE, summaryUrl: CONFIG.DEMO_MONTHLY_SUMMARY_FILE, saleSummaryUrl: CONFIG.DEMO_SALE_SUMMARY_FILE, newContractsUrl: CONFIG.DEMO_NEW_CONTRACTS_FILE, newsUrl: CONFIG.DEMO_INDUSTRY_NEWS_FILE };
    }
    return { salesUrl: CONFIG.ACTUAL_SALES_API, targetsUrl: CONFIG.SALES_TARGET_API, summaryUrl: CONFIG.MONTHLY_SUMMARY_API, saleSummaryUrl: CONFIG.SALE_SUMMARY_API, newContractsUrl: CONFIG.NEW_CONTRACTS_API, newsUrl: CONFIG.INDUSTRY_NEWS_API };
  }
  ...
  function normalizeNewsRow(raw){
    ...
    return { title, source: safeStr(raw.Source, ''), publishDate: safeStr(raw.PublishDate, ''), link: safeStr(raw.Link, ''), description: safeStr(raw.Description, ''), imageUrl: safeStr(raw.ImageUrl, '') };
  }
  ...
  function normalizeNewContractsRow(raw){
    const weekEndingDate = safeStr(raw.WeekEndingDate, '');
  ...
  async function refresh() {
    ...
    const now = new Date();
    try {
      const { salesUrl, targetsUrl, summaryUrl, saleSummaryUrl, newContractsUrl, newsUrl } = resolveUrls();
      const [salesRaw, targetsRaw, summaryRaw, saleSummaryRaw, newContractsRaw, newsRaw] = await Promise.all([
        fetchJSON(salesUrl),
      ...
      const hash = simpleHash(JSON.stringify(shipments) + '|' + JSON.stringify(targetHistory) + '|' + JSON.stringify(monthlySummary) + '|' + JSON.stringify(saleSummary) + '|' + JSON.stringify(weeklyNewContracts) + '|' + JSON.stringify(industryNews));
      const changed = hash !== lastHash;
      lastHash = hash;

      current = {
        shipments, targetHistory, targetsBySale, monthlySummary, saleSummary, weeklyNewContracts, industryNews,
