...
const DataService = (function () {
  ...
  function resolveUrls() {
    if (CONFIG.DEMO_MODE) {
      return { salesUrl: CONFIG.DEMO_ACTUAL_SALES_FILE, targetsUrl: CONFIG.DEMO_SALES_TARGET_FILE, summaryUrl: CONFIG.DEMO_MONTHLY_SUMMARY_FILE, saleSummaryUrl: CONFIG.DEMO_SALE_SUMMARY_FILE, newContractsUrl: CONFIG.DEMO_NEW_CONTRACTS_FILE, productAliasUrl: CONFIG.DEMO_PRODUCT_ALIAS_FILE, chemicalThresholdUrl: CONFIG.DEMO_CHEMICAL_THRESHOLD_FILE, newsUrl: CONFIG.DEMO_INDUSTRY_NEWS_FILE };
    }
    return { salesUrl: CONFIG.ACTUAL_SALES_API, targetsUrl: CONFIG.SALES_TARGET_API, summaryUrl: CONFIG.MONTHLY_SUMMARY_API, saleSummaryUrl: CONFIG.SALE_SUMMARY_API, newContractsUrl: CONFIG.NEW_CONTRACTS_API, productAliasUrl: CONFIG.PRODUCT_ALIAS_API, chemicalThresholdUrl: CONFIG.CHEMICAL_THRESHOLD_API, newsUrl: CONFIG.INDUSTRY_NEWS_API };
  }
  ...
  async function refresh() {
    ...
    const now = new Date();
    try {
      const { salesUrl, targetsUrl, summaryUrl, saleSummaryUrl, newContractsUrl, productAliasUrl, chemicalThresholdUrl, newsUrl } = resolveUrls();
      const [salesRaw, targetsRaw, summaryRaw, saleSummaryRaw, newContractsRaw, productAliasRaw, chemicalThresholdRaw, newsRaw] = await Promise.all([
        fetchJSON(salesUrl),
      ...
      const hash = simpleHash(JSON.stringify(shipments) + '|' + JSON.stringify(targetHistory) + '|' + JSON.stringify(monthlySummary) + '|' + JSON.stringify(saleSummary) + '|' + JSON.stringify(weeklyNewContracts) + '|' + JSON.stringify(productAliasRules) + '|' + JSON.stringify(chemicalThresholdRules) + '|' + JSON.stringify(industryNews));
      const changed = hash !== lastHash;
      lastHash = hash;

      current = {
        shipments, targetHistory, targetsBySale, monthlySummary, saleSummary, weeklyNewContracts, productAliasRules, chemicalThresholdRules, industryNews,
