...
**Your actual scenario:** the SharePoint link is a single **Excel file** (not native SharePoint Lists). This works, but the source workbook you originally uploaded (44+ per-customer tabs, header on row 2) is **not directly readable** by Power Automate's Excel connector — that connector reads one **Excel Table** (real `Insert → Table`, not just a range) inside one worksheet at a time.

**What was delivered to solve this:** `NTSF_SharePoint_Source.xlsx` — a ready-to-upload workbook with exactly the structure Power Automate needs:

| Sheet | Excel Table name | Rows | Purpose |
|---|---|---|---|
| `HuongDan` | — | — | Vietnamese instructions for whoever maintains the file (Sale Admin) |
| `ActualSales` | `ActualSales` | 765 (pre-loaded from your original file) | Every shipment, one row each |
| `TargetPlan` | `TargetPlan` | 8 (pre-loaded, incl. the 2 demo revisions) | Every target version, one row each |

**Steps:**
1. Upload `NTSF_SharePoint_Source.xlsx` to a SharePoint document library (e.g. `NTSF-Sales-Performance/Shared Documents/`), replacing or alongside the original.
2. Share the link with whoever will maintain data (Sale Admin) — see the `HuongDan` sheet inside the file for exactly how they should add/edit rows.
3. Point Power Automate at **this file's two Tables** (see §9) — not the original 44-tab file.

**Row-level versioning for `TargetPlan`:** every revision is a **new row**, never an edit to an existing row's `OriginalAllocation`. `data-service.js` groups by `Sale+Year`, sorts by `Version`, and treats the latest row's `CurrentAllocation` as current — this is exactly the convention documented in the `HuongDan` sheet.

**Security:** SharePoint's own permissions on this one file are now your access control — restrict Edit to Sales Admin/Sales Director, everyone else Read-only via the file/folder's Share settings.

---

## 9. Power Automate Design — Step by Step (Excel-in-SharePoint version)

Build two flows. Both follow the same pattern, pointed at the two different Tables in `NTSF_SharePoint_Source.xlsx`.

**Flow A — `GetActualSales`**
1. **make.powerautomate.com** → **+ Create** → **Instant cloud flow** → name it `GetActualSales` → trigger **"When an HTTP request is received"** → Create. (Leave the request body schema empty — this endpoint takes no input.)
2. **+ New step** → search **Excel Online (Business)** → action **"List rows present in a table"**.
   - **Location:** SharePoint Site (pick your site)
   - **Document Library:** the library you uploaded the file to
   - **File:** `NTSF_SharePoint_Source.xlsx`
   - **Table:** `ActualSales` (this dropdown only shows real Tables — confirms the file is set up correctly)
3. **+ New step** → **Response** action → Status Code `200` → Headers: `Content-Type: application/json` → Body: click the **body** field, then pick **"value"** from the dynamic content list of the "List rows" step (this is the array of rows — Power Automate outputs it in exactly the field-name shape needed, since the Excel Table's headers already match the JSON schema in §10).
4. Save. Open the trigger step again → copy the **HTTP POST URL** it generated.

**Flow B — `GetSalesTargets`** — identical steps, just Table = `TargetPlan`.

**Flow C — `SubmitTargetRevision`** (optional — enables the "✏️ Sửa" button in Target Maintenance to write real data instead of only simulating it in the browser):

1. **make.powerautomate.com** → **+ Create** → **Instant cloud flow** → name it `SubmitTargetRevision` → trigger **"When an HTTP request is received"** → Create.
2. This time the trigger needs a **Request Body JSON Schema** (unlike Flow A/B, this one receives data, it doesn't just get called). In the trigger's **"Request Body JSON Schema"** box, click **"Use sample payload to generate schema"** and paste:
   ```json
   {
     "TargetId": "TGT-2026-WEB-1234567890",
     "Year": 2026,
     "Sale": "Ái",
     "OriginalAllocation": 18000000,
     "CurrentAllocation": 20000000,
     "Version": 1,
     "VersionLabel": "Revision 1",
     "EffectiveDate": "2026-09-01",
     "AdjustmentReason": "Nhu cầu thị trường tăng",
     "UpdatedBy": "Giám Đốc Kinh Doanh"
   }
   ```
   Power Automate turns this into a schema automatically — this is what lets the next step offer each field as a clickable token instead of you typing expressions by hand.
3. **+ New step** → search **Excel Online (Business)** → action **"Add a row into a table"**.
   - Location: OneDrive for Business (or SharePoint Site, matching however you set up Flow A/B)
   - Document Library / File: same `NTSF_SharePoint_Source.xlsx`
   - Table: **TargetPlan**
   - A row of empty fields appears (TargetId, Year, Sale, OriginalAllocation, …) — click into **each one** and pick the matching token from the trigger (e.g. click the "TargetId" field → dynamic content panel opens → pick "TargetId"). Repeat for all 10 fields.
4. **+ New step** → **Response** → Status Code `200` → Body: type `{"status":"ok"}` (plain text is fine here, the dashboard only checks the HTTP status).
5. Save. Open the trigger step → set **"Who can trigger the flow?"** to **Anyone** (same reasoning as Flow A/B) → confirm **Method** is **POST** (this one must stay POST, since the dashboard sends data in the request body — unlike Flow A/B this is not a GET).
6. Copy the **HTTP URL** → paste into `config.js` → `SUBMIT_TARGET_REVISION_API`.

**How to verify Flow C works:** open the dashboard → Target Management → Target Maintenance → click **"✏️ Sửa"** on any Sale → the modal should show a **green** "✅ Đã kết nối API ghi dữ liệu" note (if it shows amber/orange, `SUBMIT_TARGET_REVISION_API` in `config.js` still has the placeholder value). Submit a test revision → check `NTSF_SharePoint_Source.xlsx` → `TargetPlan` sheet → a new row should appear at the bottom within seconds.

**Safety behavior:** if the write fails (network issue, wrong URL, Excel file locked for editing by someone else, etc.), the dashboard shows the exact error, **keeps the edit form open**, and does **not** apply the change anywhere — so a failed save can never silently disagree between what the dashboard shows and what's actually in the Excel file.

**⚠️ Row limit note:** "List rows present in a table" returns up to 5,000 rows in Standard connector tier by default — at 765 rows today you're well within range; if `ActualSales` grows past that, add pagination (`Top Count`/`Skip` in a loop) or filter server-side to the last 1–2 years.

**How the live-edit loop actually works (this is what you asked):**
1. Sale Admin opens `NTSF_SharePoint_Source.xlsx` on SharePoint (browser or desktop Excel, synced via OneDrive) and edits/adds rows directly inside the `ActualSales` or `TargetPlan` **Table**.
2. They save (Ctrl+S, or autosave if using the web/desktop app with SharePoint sync). No export, no re-upload, no code — the file **is** the database now.
3. Flow A/B do **not** cache anything — every single call to their HTTP URL re-reads the Table live at that instant.
4. The dashboard calls those URLs automatically every 5 minutes (or immediately if the user clicks 🔄). The next call after the save picks up the new data.
5. **Nothing else needs to change** — not `config.js`, not the dashboard code, not GitHub Pages. This is the entire point of the API-driven architecture.
...
- `ShipmentDate`/`EffectiveDate` must be `YYYY-MM-DD` (or start with it); anything else is treated as blank. Excel serial-date numbers (e.g. `46036`) returned by some Excel Online connector configurations are also auto-detected and converted.
...
### `POST {SUBMIT_TARGET_REVISION_API}` (optional — Flow C)
Request body sent by the "✏️ Sửa" form:
```json
{
  "TargetId": "TGT-2026-WEB-1721540000000",
  "Year": 2026, "Sale": "Ái",
  "OriginalAllocation": 18000000, "CurrentAllocation": 20000000,
  "Version": 1, "VersionLabel": "Revision 1",
  "EffectiveDate": "2026-09-01",
  "AdjustmentReason": "Nhu cầu thị trường tăng",
  "UpdatedBy": "Giám Đốc Kinh Doanh"
...
```
Expected response: any `200` status. The dashboard doesn't parse the response body for this endpoint — only the status code matters. Any non-2xx status is treated as a failed save (see §9 Flow C "Safety behavior").
...
- **Target Maintenance**: mỗi dòng có nút **✏️ Sửa** — mở form nhập Phân Bổ Mới, Ngày Hiệu Lực, Lý Do, Người Cập Nhật. Modal hiện rõ trạng thái: nếu đã cấu hình `SUBMIT_TARGET_REVISION_API` (Flow C, §9), thay đổi được **ghi thật vào SharePoint/OneDrive** và đồng bộ cho mọi người dùng; nếu chưa cấu hình, thay đổi chỉ **áp dụng tạm thời** trong phiên trình duyệt hiện tại (mất khi tải lại trang). Nếu ghi thất bại, dashboard giữ nguyên form và báo lỗi rõ ràng, không âm thầm mất dữ liệu. **Revision History** hiển thị lại toàn bộ, có thể lọc theo Sale/khoảng ngày.
...
3. Target Maintenance **hỗ trợ ghi thật vào SharePoint/OneDrive** khi đã cấu hình Flow C (`SUBMIT_TARGET_REVISION_API` trong `config.js`, hướng dẫn ở §9). Nếu chưa cấu hình, dashboard tự động chuyển về chế độ mô phỏng phía trình duyệt (an toàn, không lỗi) — modal sẽ hiện rõ đang ở chế độ nào trước khi anh bấm Lưu.
